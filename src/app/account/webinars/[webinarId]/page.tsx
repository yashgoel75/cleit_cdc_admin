...
async function exportToExcelGrouped(
  ...
  fileName: string = "webinar_students_grouped.xlsx"
) {
...
export default function WebinarStudentsPage() {
  ...
  useEffect(() => {
    const fetchStudents = async () => {
      ...
      try {
        const res = await fetch(`/api/getWebinarStudents/${webinarId}/students`, {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
          },
        });
        if (!res.ok) throw new Error("Failed to fetch webinar students.");
        ...
        setError("Failed to fetch webinar students.");
      } finally {
        setLoading(false);
      }
    };
          ...
          <button
            onClick={() =>
              exportToExcelGrouped(
                groupedStudents,
                `Webinar_${webinarId}_Students.xlsx`
              )
            }
